...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    #    small_int_test = numbers[1]
    #    if numbers[index] < small_int_test:
    #        small_int_test = numbers[index]
    #    return small_int_test

    if numbers == []:
        return None
    small_int_test = numbers[0]
    for number in numbers:
        if number < small_int_test:
            small_int_test = number
    return small_int_test
...
def largest_int(numbers):
    ...
    **DO NOT USE** the built-in function `max()`!
    ...
    if numbers == []:
        return None
    large_int_test = numbers[0]
    for number in numbers:
        if number > large_int_test:
            large_int_test = number
    return large_int_test
...
def average(numbers):
    ...
    for number in numbers:
        string_count += 1
        sum_string += number
    mean = float(sum_string) / string_count
    return mean
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    if items == []:
        return
    len_to_del = len(items)
    for item in items[::-1]:
        items.append(item)
    del items[0:len_to_del]
    return items
...
def duplicates(items):
    ...
    for dupl in dupl_list:
        if dupl_list.count(item) > 1:
            dupl_list.remove(item)
    return dupl_list
